...
export default function ProductCard({ item }: Props) {
  return (
    <Link
      ...
      style={{ borderColor: 'var(--store-line)', background: 'var(--store-paper)' }}
    >
      <div className="relative" style={{ aspectRatio: '1 / 1' }}>
        ...
        {item.is_kit && (
          <span
            ...
            style={{ background: 'var(--store-purple)', color: '#fff' }}
          >
            Kit
          </span>
        )}
      ...
      <div className="p-2.5">
        <p className="font-extrabold text-sm leading-tight truncate" style={{ color: 'var(--store-ink)' }}>
          {item.name}
        </p>
        {item.rental_price_unit != null && (
          <p className="text-sm font-black mt-1" style={{ color: 'var(--store-teal-d)' }}>
